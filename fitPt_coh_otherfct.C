...
Double_t sum(Double_t *x, Double_t *par)
{
    return PowerLaw(x,par) + TailExpo(x, &par[4]);
}
*/
void fitPt_coh_otherfct()
{
 ...
 LogNormalfct->SetLineColor(kRed);
 ...
 TF1* GlobalFit = new TF1("GlobalFit", sum, 0.,1.,5);
 GlobalFit->SetParameters(1.,1.,1. ,1.,1.);
 ...
 h3->SetTitle("Fit Log Normal + TailExpo - Pt Distribution - mass range [3.4 - 5] - coh; p_{T} GeV/c; # DiMuons");
 //GlobalFit->SetParameters(1.,1.,1.,1.,1.,1.,1.);
 //h3->Fit("GlobalFit","R","");
 ...
 TCanvas* c1 = new TCanvas("FitLogNormalTailExpo_Pt Distribution_3.4_5._coh","mass fct pt coh");
 c1->cd();
 gPad->SetLogy();
 ...
 const char *formula = "f( p_{T} ) = #frac{p0}{2#pi p2 p_{T}} exp#left( - #frac{ln^{2}#left(p_{T} - p1#right)}{2 p2^{2}} #right)+ p3 p_{T} exp#left(- #frac{p_{T}}{p4}#right)";
 TLatex latex;
 latex.SetTextSize(0.025);
 latex.DrawLatex(.1,100.,formula);
 
 GlobalFit->GetParameters(par);
 LogNormalfct->SetParameters(par);
 //Gaussianfct->SetParameters(&par[3]);
 Tail->SetParameters(&par[3]);
 LogNormalfct->Draw("same");
