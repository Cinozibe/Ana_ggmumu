...
void plot_lego(){
 ...
 Double_t nR_hadro = 15.323;
  ...
  //Parameters for mass range [1.9-5]

 Fitfct->FixParameter(0,47.53);
 ...
 */
 ...
 Fitfct->FixParameter(5,0.00059688);    //fcoh p0 norm
 
 Fitfct->FixParameter(6,0.000319);   //fincoh p0 norm

//fhadro p0
 Fitfct->SetParLimits(7,569.3,589.7);
// Fitfct->SetParLimits(7,50.,589.7);

 Fitfct->FixParameter(8,-130.8); //gg contribution
 Fitfct->FixParameter(9,127.5);
 Fitfct->FixParameter(10,-22.57);
 Fitfct->FixParameter(11,-4.394);
 Fitfct->FixParameter(12,0.5879);
 Fitfct->FixParameter(13,0.2604);
 Fitfct->FixParameter(14,-0.03669);

/*
 Fitfct->SetParameters(8,1.043e4);     //gg contribution
 Fitfct->SetParameters(9,-1.92e4);
 Fitfct->SetParameters(10,1.292e4);
 Fitfct->SetParameters(11,-3.509e3);
 Fitfct->SetParameters(12,85.05);
 Fitfct->SetParameters(13,150.9);
 Fitfct->SetParameters(14,-29.43);
 Fitfct->SetParameters(15,1.745);
*/

 //coherent jpsi contribution
 Fitfct->FixParameter(15,171.4);
 ...
 //Fitfct->FixParameter(22,2.661);       //incoherent jpsi contribution
 Fitfct->FixParameter(22,26.61);
 ...
 Fitfct->SetParLimits(29,0.1,10.);    //hadronic jpsi contribution
...
// make the 3D fit
 ...
 TH1D* hData = (TH1D*) fHistoDiMuonOS->Projection(1);
 hData->SetLineColor(kBlue+1);
 hData->SetMarkerStyle(20);
 hData->SetMarkerSize(0.4);

 TH1D* hData_mass = (TH1D*) fHistoDiMuonOS->Projection(0);
 ...
 CBE_incoh->SetLineWidth(4);
 CBE_incoh->SetLineColor(kGreen+1);

 TF1* CBE_hadro = new TF1("CBE_hadro",CrystalBallExtended,1.9,5.,7);
 CBE_hadro->SetParameters(1.,1.,1.,1.,1.,1.,1.);
 CBE_hadro->SetLineStyle(4);
 CBE_hadro->SetLineWidth(4);
 CBE_hadro->SetLineColor(kYellow+1);
 ...
 fgamma->GetParameters(&par[8]);
 CBE_coh->GetParameters(&par[15]);
 CBE_incoh->GetParameters(&par[22]);
 CBE_hadro->GetParameters(&par[29]);
...
// comparison between data and pt distribution 3D fit
 ...
 hRandFit_Pt->FillRandom("Fitfct",8777);
...
//comparison between data and mass distribution 3D fit
 ...
 hRandFit_Mass->FillRandom("Fitfct",8777);
 ...
 c4->cd();
 h3->Draw("e");
 h3->GetYaxis()->SetRangeUser(0,1000);
